...
class WebStorageService {
  ...
  async updateMessageStatus(messageId: string, status: Message['status']): Promise<void> {
    try {
      ...
      if (messageData) {
        ...
        localStorage.setItem(key, JSON.stringify(message));
        console.log('Message status updated in localStorage:', messageId, status);
      }
    } catch (error) {
  ...
  async clearOldMessages(daysToKeep: number = 30): Promise<void> {
    try {
      ...
      console.log('Old messages cleaned up from localStorage');
    } catch (error) {
  ...
  async deleteChatFromCache(chatId: string): Promise<void> {
    try {
      ...
      console.log('Chat deleted from localStorage cache:', chatId);
    } catch (error) {
  ...
  async removeUserFromChat(chatId: string, userId: string): Promise<void> {
    try {
      ...
      console.log('User removed from chat in localStorage cache:', chatId, userId);
    } catch (error) {
...
class PlatformStorageService {
  ...
  async initialize(): Promise<void> {
    if (Platform.OS === 'web') {
      console.log('Web storage service initialized');
      return;
    }
