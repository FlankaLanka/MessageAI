...
class NetworkService {
  ...
  private updateState(state: any) {
    ...
    console.log('🌐 Network state update:', {
      ...
      newState: newState,
      changed: JSON.stringify(newState) !== JSON.stringify(this.currentState)
    });

    // Only update if state actually changed
    if (JSON.stringify(newState) !== JSON.stringify(this.currentState)) {
      this.currentState = newState;
      console.log('🌐 Network state changed, notifying listeners');
      this.notifyListeners();
    } else {
      console.log('🌐 Network state unchanged, skipping notification');
    }
  }
